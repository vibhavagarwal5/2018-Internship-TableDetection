...
def MetricsCalcul(T,Table_CSV_Name,Table_Annotation,Table_IsAnnotated):
	...
	for k in Table_Annotation:
		Table_Annotation[k] = ["http://www.wikidata.org/entity/Q"+str(top3Ann) for top3Ann in Table_Annotation[k] if not top3Ann == None ]
				
		# print("Table_Annotation[k]:",Table_Annotation[k])

		#look in mapping file, for the corresponding dbpedia link
		with open('/home/yasamin/Codes/WebTableAnnotation/data/T2D/T2D_DBpedia2Wikidata2.csv', 'r',encoding='utf-8') as csvFile:
			csv_reader = csv.reader(csvFile, delimiter=',')
			for row in csv_reader:
				try:
					#Table_Annotation has 1 to 3 results now.
					if(row[1] in Table_Annotation[k]):
						...
						#mapped_prediction can have multiple links 
						if(k in mapped_Prediction_Dict):
							if not(row[0] in mapped_Prediction_Dict[k]):
								#not to add duplicates exist in mapping file
								mapped_Prediction_Dict[k].append(row[0])
						else:
	...
	for index, row in T.iterrows():
		try:
			...
			#cause prediction hast a list of top 3 results.
			elif not(dict_final_true[index] in dict_final_pred[index]):
				FP = FP + 1
			elif(dict_final_true[index] in dict_final_pred[index]):
				print("they are equal (result is among top 3): ", index)
				TP = TP + 1
			else:
