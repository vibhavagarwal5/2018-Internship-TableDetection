...
def RunHybrid(info_data, TableList):
	logging.debug("I am in Hybrid of Limaye")
	# logging.debug(str(info_data['domain']) +":::"+ str(info_data['Levenshtein_Limaye']))
	...
	tables_dir = '/home/vibhav/bar/data_csv/' + info_data['domain'] 
	entity_dir = '/home/vibhav/bar/data_csv_out/' + info_data['domain']
	...
	for table_csv in TableList:
			logging.debug("This is the Table file name : " + table_csv)
		...
		# with open(table_csv, 'r', encoding='utf-8') as csvTableFile:
			T = pd.read_csv(tables_dir + '/' + table_csv, header=None)
		...
		#----------------------------------------------------------------
			# RLU_TPrimeIsAnnotated , RLU_TPrimeAnnotation = fp.getFullPhase(T,table_csv, info_data)
		...
		#----------------------------------------------------------------
			Emb_TPrimeIsAnnotated, Emb_TPrimeAnnotation = emb.getEmbedding(T, info_data)

			logging.debug('Time1 hybrid Limaye: ' + format(time.time() - start_time))

			logging.debug("\nEmbedding result:")
			logging.debug("-"*30)
			logging.debug(Emb_TPrimeAnnotation)
			logging.debug(Emb_TPrimeIsAnnotated)
			T['new_col'] = Emb_TPrimeAnnotation.values()
			logging.debug(T)
			T.to_csv(entity_dir + "/" + table_csv,header=False,index=False)

			logging.debug('Time2 hybrid Limaye: ' + format(time.time() - start_time))
	...
	# print("-"*30)	
	logging.debug("\n-----------------------------------------------------------------------------------")
	logging.debug("Run time for 1 lookup phase and one embedding phase, Local Endpoint " + format(time.time() - start_time))
	logging.debug("-----------------------------------------------------------------------------------\n")
	return
