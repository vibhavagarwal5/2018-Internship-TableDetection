""" Creating Levenshtein and Type Pickle files only for all the columns and entities"""
...
Levenshtein_Limaye = pickle.load(open("/home/yasamin/Codes/WebTableAnnotation/data/LimayeLevenshtein.pickle", "rb"))    
    ...
    for index, row in T.iterrows():
        for tcol in T.keys():
			...
			#results= search.SearchWithDistanceSF(label)
                ...
                try: 
                    #get from labelcolumn levenshtein dict
                    results = Levenshtein_Limaye[label]
                    print("result is:",results,"for label:",label)
                except KeyError:
                    #couldnt find in labelcolumn levenshtein dict
                    try:
...
def Multiprocess_Limaye():
    ...
    with open('/home/yasamin/Codes/WebTableAnnotation/data/LimayeTypes_allcols.pickle', 'wb') as handle:
        pickle.dump(InstanceOfDict, handle)
    #save dictionary of Levenshtein code to pickle file
    with open('/home/yasamin/Codes/WebTableAnnotation/data/LimayeLevenshtein_allcols.pickle', 'wb') as handle:
...
def Multiprocess_T2D():
    ...
    #save dictionary of label:instanceof to pickle file
    with open('/home/yasamin/Codes/WebTableAnnotation/data/T2DTypes_allcols.pickle', 'wb') as handle:
        pickle.dump(InstanceOfDict, handle)
    #save dictionary of Levenshtein code to pickle file
    with open('/home/yasamin/Codes/WebTableAnnotation/data/T2DLevenshtein_allcols.pickle', 'wb') as handle:
        pickle.dump(LevenshteinDict, handle)
